...
extern struct proc* proc_list[12000];
...
userinit(void)
{
  ...
  */
  ...
  release(&ptable.lock);
  // queue_push(my_scheduler->mlfq[0], p);
...
// there's no process.
// The upper locking is not work, so I can not change this sched code.
void
sched(void)
{
  int intena;
